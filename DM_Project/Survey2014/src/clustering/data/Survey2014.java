	...
	public String analyzeClusters(ArrayList<KMeanCluster> clusters, int k){
		...
		for (int i = 0; i<k; i++){
			...
			for (Object counter : counters_Prog_skill.keySet()){
				if (counters_Prog_skill.get(counter)/size*100 > 0)
					result += "\nProg_skill "+counter.toString()+": "+counters_Prog_skill.get(counter)/size*100;
			}
			for (Object counter : counters_Uni_yrs.keySet()){
				if (counters_Uni_yrs.get(counter)/size*100 > 0)
					result += "\nUni_yrs "+counter.toString()+": "+counters_Uni_yrs.get(counter)/size*100;
			}
			for (Object counter : counters_EngSkill.keySet()){
				if (counters_EngSkill.get(counter)/size*100 > 0)
					result += "\nEngSkill "+counter.toString()+": "+counters_EngSkill.get(counter)/size*100;
			}
		}
		return result;
	}
