package frequentPattern.apriori;
...
public class Apriori {
    ...
    public static ArrayList<AssociationRule> createAssociationRules(Hashtable<ItemSet,Integer> frequentItemSets, double confidenceThreshold, int transactionCnt){
        ...
        for (ItemSet itemSetAB : frequentItemSets.keySet()){
    	   if (itemSetAB.getSet().length > 1)
	    	   for (ItemSet itemSetB : frequentItemSets.keySet()){
	        	   if (!itemSetAB.equals(itemSetB) && itemSetAB.getSet().length > itemSetB.getSet().length){
	        		   HashSet<Integer> setAB = new HashSet<Integer>(Arrays.asList(itemSetAB.getSet()));
	        		   HashSet<Integer> setB = new HashSet<Integer>(Arrays.asList(itemSetB.getSet()));
    		...
    		for (ItemSet itemset2: lowerLevelItemSets.keySet()){
    			...
    			if (joined != null){
    				if (!candidates.containsKey(joined)){
    					int support = countSupport(joined.getSet(),transactions);
    ...
    private static ItemSet joinSets( ItemSet first, ItemSet second ) throws Exception {
        ...
        if (!first.equals(second)){
	        ...
	        if (second.getSet()[k-1] < first.getSet()[k-1]){
	    		set[k-1] = second.getSet()[k-1];
	    		set[k] = first.getSet()[k-1];
	        }
	    	else{
	    		set[k-1] = first.getSet()[k-1];
	    		set[k] = second.getSet()[k-1];
	    	}
	        if (skip)
	        	return null;
	        else
